...
class Login(webapp.RequestHandler):
  ...
  def get(self):   
        try :
            authResult = userStore.authenticate(self.request.get('username'), self.request.get('password'),
        ...
        try:               
            ...
            for notebook in notebooks:          
                ...
                if self.request.get('type') == 'Todo':
                    filter.words = "any: todo:false tag:todo"
                else:
                    filter.words = "any: todo:true tag:todo"     
            ...
            return self._simple_error(str(sys.exc_info()[1]), code=500)
